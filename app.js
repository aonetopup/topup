...
let buktiBayarFile = null;  // file bukti bayar yang dipilih pelanggan
...
function selectPay(id) {
  ...
  // Reset bukti jika ganti metode bayar
  buktiBayarFile = null;
  ...
  // Konfirmasi tetap disabled sampai ada bukti
...
function handleBuktiUpload(input) {
  ...
  if (file.size > 10 * 1024 * 1024) {
    showUbStatus('error', '❌ Ukuran foto terlalu besar. Maks. 10 MB.');
    return;
  }

  // Validasi tipe file
  if (!file.type.startsWith('image/')) {
    showUbStatus('error', '❌ File harus berupa gambar (JPG, PNG, dll).');
    return;
  }
  ...
  reader.onload = (e) => {
    document.getElementById('ub-preview-img').src = e.target.result;
    document.getElementById('ub-preview-name').textContent = '✅ ' + file.name + ' (' + (file.size/1024).toFixed(0) + ' KB)';
    document.getElementById('ub-preview-wrap').style.display = 'flex';
    document.getElementById('ub-dropzone').style.display = 'none';
    showUbStatus('success', '✅ Foto siap dikirim. Klik Konfirmasi untuk mengirim ke admin.');
    document.getElementById('btn-confirm').disabled = false;
  };
  reader.readAsDataURL(file);
}

function showUbStatus(type, msg) {
  const el = document.getElementById('ub-status');
  el.className = 'ub-status ' + type;
  el.textContent = msg;
  el.style.display = 'block';
}

/**
 * Kirim foto bukti bayar ke Telegram via sendPhoto
 */
async function sendBuktiToTelegram(orderId, gd, idGame, nick, wa) {
  if (!buktiBayarFile) return false;
  if (!CFG.tgBotToken || CFG.tgBotToken === 'GANTI_TELEGRAM_BOT_TOKEN') return false;

  try {
    const caption = [
      `📷 *BUKTI BAYAR — ONE TopUp*`,
      `━━━━━━━━━━━━━━━━━━━`,
      `Order ID  : *${orderId}*`,
      `Game      : ${gd.name}`,
      ...
      `No. WA    : ${wa}`,
      `━━━━━━━━━━━━━━━━━━━`,
      `💡 Verifikasi dan klik Konfirmasi Bayar jika sesuai.`
    ].filter(Boolean).join('\n');

    const formData = new FormData();
    formData.append('chat_id', CFG.tgChatId);
    formData.append('photo', buktiBayarFile, buktiBayarFile.name);
    formData.append('caption', caption);
    formData.append('parse_mode', 'Markdown');
    formData.append('reply_markup', JSON.stringify(buildInlineKeyboard(orderId)));

    const res = await fetch(`https://api.telegram.org/bot${CFG.tgBotToken}/sendPhoto`, {
      method: 'POST',
      body: formData
    });
    ...
    console.warn('sendPhoto gagal:', e);
    return false;
  }
}
...
async function submitOrder() {
  ...
  showLoading('Mengirim order & bukti bayar...');
  ...
  // Kirim foto bukti bayar ke Telegram (jika ada)
  let photoSent = false;
  if (buktiBayarFile) {
    showLoading('📷 Mengirim foto bukti bayar ke admin...');
    photoSent = await sendBuktiToTelegram(orderId, gd, idGame, nick, wa);
  }

  // Kirim notif teks order baru ke Telegram (jika foto gagal atau tidak ada foto)
  let tgMsgId = null;
  if (!photoSent && CFG.tgBotToken && CFG.tgBotToken !== 'GANTI_TELEGRAM_BOT_TOKEN') {
    try {
      showLoading('📨 Mengirim notifikasi order...');
  ...
  if (buktiBayarFile) rows.push(['Bukti Bayar', photoSent ? '✅ Terkirim ke admin' : '⚠️ Gagal dikirim otomatis']);

  document.getElementById('s-card').innerHTML = rows.map(([k,v]) =>
    `<div class="s-row"><span class="s-key">${k}</span><span class="s-val">${v}</span></div>`
  ).join('');

  // Update subtitle step 4 sesuai status foto
  setTimeout(() => {
    const sSubText = document.getElementById('s-sub-text');
    if (sSubText) {
      if (photoSent) {
        sSubText.innerHTML = '📷 Foto bukti bayar + notifikasi sudah dikirim ke admin.<br>Tunggu konfirmasi dari admin.';
      } else if (buktiBayarFile) {
        sSubText.innerHTML = '⚠️ Foto gagal otomatis. Kirim manual via WA/Telegram.<br>Notifikasi order sudah terkirim.';
      }
    }
  }, 100);

  // Reset state bukti bayar
  buktiBayarFile = null;
